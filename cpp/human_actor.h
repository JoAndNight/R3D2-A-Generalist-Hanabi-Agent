...
class HumanActor {
 public:
  HumanActor(int numPlayer, int playerIdx)
      : numPlayer_(numPlayer)
      , playerIdx_(playerIdx)
      , stage_(Stage::ObserveBeforeAct) {
  }

  void setPartners(std::vector<std::shared_ptr<HumanActor>> partners) {
 ...
 private:
  ...
  // Helper functions for displaying game state
  void printGameState(const HanabiEnv& env);
  void printLegalMoves(const std::vector<hle::HanabiMove>& legalMoves);
  int getUserActionChoice(const std::vector<hle::HanabiMove>& legalMoves);

  const int numPlayer_;
  const int playerIdx_;
  std::vector<std::shared_ptr<HumanActor>> partners_;
